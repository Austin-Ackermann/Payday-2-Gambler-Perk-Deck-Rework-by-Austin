-- init
Hooks: PostHook(AmmoClip, "init", "ammoclip_init_byp", function(self) 
    self.gambler_lucky_dodge_stacks = 0 
    self.gambler_dodge_difference = 0
    self.gambler_unlucky_cap = tweak_data.upgrades.values.player.gambler_unlucky_cap
    self.gambler_max_lucky_dodge_stacks = tweak_data.upgrades.values.player.gambler_max_lucky_dodge_stacks
    self.gambler_lucky_dodge_incriment = tweak_data.upgrades.values.player.gambler_lucky_dodge_incriment
    self.gambler_lucky_dodge_decriment = tweak_data.upgrades.values.player.gambler_lucky_dodge_decriment
end)

local player_manager = managers.player

Hooks: PostHook(AmmoClip, "init", function(self, unit) 
    self.gambler_dodge_difference = 0
        ...
        if not unit:character_damage():is_downed() and player_manager:has_category_upgrade("temporary", "loose_ammo_restore_health") and not player_manager:has_activate_temporary_upgrade("temporary", "loose_ammo_restore_health") then
            ...
            if player_manager:has_category_upgrade("player", "gambler_dodge") then
                ...
                for id, weapon in pairs(inventory:available_selections()) do
                    local weapon1ammo = weapon.unit:base():ammo_info()
                end
                local current_player_ammo_percentage = self.ammo_amount / self.max_ammo -- this is a whole number, meaning this is probabnly just a number. -- eg. 30 AKA 30% in game
                -- check this, not sure if self.max_ammo exists. 
                
                
                -- the higher the current ammo, the higher the chance of being unlucky. Caps at 50% chance of being unlucky at 50% or more current__player_ammo
                ...
                -- todo: 
                -- save and pass the variables corectly - done
...
function player_manager:skill_dodge_chance(...)
    local chance = self.gambler_dodge_difference / 100
    
    -- self.player_dodge_chance = self.player_dodge_chance - (old_lucky_dodge_stacks / 100) + (new_lucky_dodge_stacks / 100)
